...
public:
	...
	void RemoveEdges()
	{
		...
		{
			for (std::list<edge_type *>::const_iterator edge = it->begin(); edge != it->end(); ++edge)
			{
				delete *edge;
			}
	...
	public:
		...
		ConstEdgeIterator& operator=(const ConstEdgeIterator&){ return *this; }
...
void SparseGraph<node_type, edge_type>::AddEdge(EdgeType * edge)
{
		...
		(m_Nodes[edge->From()]->Index() != invalid_node_index))
	{
		...
		if (!m_bDigraph)
		{
			...
			{
				EdgeType * newEdge = new EdgeType(edge->From(), edge->To(), edge->Cost());

				newEdge->SetTo(edge->From());
				newEdge->SetFrom(edge->To());

				m_Edges[newEdge->To()].push_back(newEdge);
			}
		}
	}
}
	...
	if (!m_bDigraph)
	{
			...
			++curEdge)
		{
			if (curEdge->To() == from){ 
				...
				for (std::list<edge_type *>::const_iterator edge = curEdge->begin(); edge != curEdge->end(); ++edge)
				{
					delete *edge;
				}
...
void SparseGraph<node_type, edge_type>::CullInvalidEdges()
{
	for (EdgeListVector::iterator curEdgeList = m_Edges.begin(); curEdgeList != m_Edges.end(); ++curEdgeList)
	{
		for (EdgeList::iterator curEdge = (*curEdgeList).begin(); curEdge != (*curEdgeList).end(); ++curEdge)
		{
			...
			{
				for (std::list<edge_type *>::const_iterator edge = curE->begin(); edge != curE->end(); ++edge)
				{
					delete *edge;
				}
...
{
	for (EdgeList::const_iterator edge = m_Edges[from].begin();
		edge != m_Edges[from].end();
		++edge)
	{
		if ((*edge)->To() == to)
		{
